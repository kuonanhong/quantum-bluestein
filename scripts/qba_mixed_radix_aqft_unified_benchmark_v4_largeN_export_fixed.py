...
def save_circuit_png(circuit: Any, filename: str) -> str:
    ...
    try:
        ...
        save_figure_tight(fig, str(Path(filename).with_suffix('')), dpi=600, pad_inches=0.02, save_pdf=True, save_svg=False)
        if plt is not None:
            plt.close(fig)
...
def save_fidelity_plot(rows: Sequence[BenchmarkRow], filename: str) -> None:
    ...
    fig.tight_layout()
    save_figure_tight(fig, str(Path(filename).with_suffix('')), dpi=600, pad_inches=0.02, save_pdf=False, save_svg=False)
    try:
