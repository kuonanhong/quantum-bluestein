...
#
# This file is a modified version of the N=3 Quantum Bluestein worked-example
...
#   - paper-aligned input state for the Appendix A N=3 worked example;
#   - post-selected logical-output histogram export;
#   - additional resource and validation diagnostics.
#
# Distributed under the MIT License.
"""
General revised proof-of-concept QBA script.
Worked-example aligned version for Appendix A, N=3.
...
def main() -> None:
    ...
    # === FIXED: Save circuit figure automatically ===
    try:
        fig_circ = qc.draw(output='mpl', scale=0.6)
        circ_filename = f'qba_circuit_revised_N{args.N}.png'
        fig_circ.savefig(circ_filename, dpi=300, bbox_inches='tight')
        plt.close(fig_circ)
        print(f"\n[Visual] Circuit diagram successfully saved as '{circ_filename}'")
    ...
    if args.aa_demo:
        ...
        try:
            ...
            try:
                fig_aa = aa_circ.draw(output="mpl", scale=0.45)
                aa_filename = f"qba_amplitude_amplification_demo_N{args.N}.png"
                fig_aa.savefig(aa_filename, dpi=300, bbox_inches="tight")
                plt.close(fig_aa)
                print(f"[Visual] Amplitude-amplification demo circuit saved as '{aa_filename}'")
            except Exception as e:
    ...
    if args.hardware:
        ...
        try:
            ...
            fig_hw.savefig(hw_filename, dpi=300, bbox_inches='tight')
            plt.close(fig_hw)
